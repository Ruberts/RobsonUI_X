...
local TRINKET_FILTER = {
		...
		--Proccs
		CreateSpellEntry( 71975 ), -- Bloodlust
		CreateSpellEntry( 78151 ), -- Heroism
		CreateSpellEntry( 80353 ), -- TimeWarp
		CreateSpellEntry( 90355 ), -- Ancient Hysteria, bloodlust from hunters pet
		CreateSpellEntry( 26297 ), -- Berserking
		CreateSpellEntry( 57933 ), -- Tricks of Trade
		CreateSpellEntry( 91810 ), -- License to Slay
		CreateSpellEntry( 91832 ), -- Fury of Angerforge
		CreateSpellEntry( 125487 ), -- Lightweave
		CreateSpellEntry( 128985 ), -- Blessings of the Celestials (Relic of Yu'lon)
		CreateSpellEntry( 126577 ), -- Inner Brilliance (Light of the Cosmos)
...
local CLASS_FILTERS = {
		...
		MAGE = {
			...
			player = {
				...
				CreateSpellEntry( 131078 ), -- Icy Veins (Glyph)
				CreateSpellEntry( 130 ), -- Slow Fall
				CreateSpellEntry( 116257 ), -- Invoker's Energy
				CreateSpellEntry( 116267 ), -- Incanter's Absorption
				CreateSpellEntry( 115610 ), -- Temporal Shield
				CreateSpellEntry( 12043 ), -- Presence of Mind
...
do
	...
	do
		...
		CreateAuraBar = function( parent )
			...
			icon:Point( iconAnchor1, result, iconAnchor2, iconOffset * -8, 3 )
			...
			local ibackground = CreateFrame( "Frame", ibackground, result )
			ibackground:SetFrameStrata("Background")
			ibackground:Point("TOPLEFT", icon, "TOPLEFT", 3, -3);
			ibackground:Point("BOTTOMRIGHT", icon, "BOTTOMRIGHT", -3, 3);
			ibackground:RobSkin()
			...
			bar:Point("TOPLEFT", result, "TOPLEFT", 8, 0 );
			bar:Point("BOTTOMRIGHT", result, "BOTTOMRIGHT", 0, 0);
			result.bar = bar;
			
			-- Bar border 
			local background = CreateFrame("StatusBar", nil, bar, nil);
			background:SetFrameStrata("Background")
			background:Point("TOPLEFT", bar, "TOPLEFT", 0, 0);
			background:Point("BOTTOMRIGHT", bar, "BOTTOMRIGHT", 0, 0);
			background:RobSkin()
